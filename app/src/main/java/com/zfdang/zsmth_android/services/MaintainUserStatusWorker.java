...
public class MaintainUserStatusWorker extends Worker {
    ...
    public Result doWork() {
        ...
        try {
            doWorkInternal(latch);
        } catch (Exception e) {
            latch.countDown(); // 确保即使发生异常也释放锁存器
        }
    ...
    private void doWorkInternal(CountDownLatch latch) {
        ...
        helper.wService.queryActiveUserStatus()
                .onErrorResumeNext(throwable -> {
                    if (throwable instanceof HttpException) {
                        ...
                        if (httpEx.code() == 500) {
                            Log.w(TAG, "HTTP 500 error occurred, treating as success and using username from settings");
                            ...
                            boolean updateUserIcon = !android.text.TextUtils.equals(userStatus.getId(), SMTHApplication.displayedUserId);
...
*/
                    ...
                    public void onError(Throwable e) {
                        // 在这里处理其他非 500 错误
                        // 由于 500 错误已在 onErrorResumeNext 中处理，此处不会收到 500 错误
                        // 释放锁存器以确保 Worker 能够完成
                        latch.countDown();
                    }
